...
| The exact author-resolution priority order (`ZPARTNERNAME` → `ZWAPROFILEPUSHNAME` → `ZWAGROUPMEMBER.ZCONTACTNAME`) is externally established WhatsApp behavior. | `externally conflicted` | WhatsApp Web evidence conflicts with the claim as written. The visible behavior supports some parts of the strategy, but not a single fixed table order in every case. |
