...
final class ErrorHandlingTests: XCTestCase {
    ...
    func testGetBackupsReportsIncompleteBackupAsInvalid() throws {
        let rootURL = try TestSupport.makeTemporaryDirectory(prefix: "SwiftWABackupAPI-invalid-backup")
        defer { try? TestSupport.removeItemIfExists(at: rootURL) }
    ...
    func testConnectChatStorageDbRejectsUnsupportedSchema() throws {
        let fixture = try TestSupport.makeTemporaryBackup { db in
    ...
    func testFetchWAFileHashThrowsWhenMediaIsMissing() throws {
        let fixture = try TestSupport.makeTemporaryBackup { _ in }
        defer { try? TestSupport.removeItemIfExists(at: fixture.rootURL) }
